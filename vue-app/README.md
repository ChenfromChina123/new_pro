...
- **云盘文件夹树优化**: 文件夹树按层级深度自适应缩进，超过 3 层自动启用滚动条，展开/折叠加入平滑过渡并兼容移动端与桌面端，点击目录项仅当前项高亮为蓝色。
...
- ✅ 公共词库默认加载前50条，分页按需加载
- ✅ **AI 文章生成向导**：
  - 支持选择来源单词表及具体单词（全选/多选）
  - 自定义文章难度（初/中/高）和长度（短/中/长）
  - AI 智能生成主题建议（Title）
  - 生成包含目标词汇的短文，并自动加粗高亮
- ✅ **学习资料下载**：支持将生成的 AI 文章导出为 HTML 或 PDF，附带重点词汇表
