...
- **AI 终端助手升级**:
  - **任务链可视化**: 新增可折叠、可拖拽排序的任务链侧边栏，实时展示任务进度与状态（进行中/已完成/等待中）。
  - **会话分组与定位**: 实现基于任务的消息自动分组，支持点击任务节点平滑滚动定位到对应消息上下文。
  - **交互体验增强**: 引入虚拟滚动技术优化长列表性能（1000+消息流畅），支持任务链进度条展示及 Markdown 思考过程折叠。
  - **结构化响应**: 规范 AI 响应格式，支持思考过程（Thought）与执行步骤（Steps）的结构化展示。
- **UI 交互与反馈优化**:
