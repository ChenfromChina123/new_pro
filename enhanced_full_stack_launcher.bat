...
findstr /R /C:"^[MADRCU]" temp_git_status.txt >nul
if !errorlevel! equ 0 (
    echo [操作] 检测到本地更改，正在暂存...
    for /f "tokens=*" %%d in ('date /t') do set "date_part=%%d"
    for /f "tokens=*" %%t in ('time /t') do set "time_part=%%t"
    git stash push -m "Auto-stash before update !date_part!_!time_part!"
)
...
for /f "tokens=2,5" %%a in ('netstat -ano ^| findstr ":5000.*LISTENING"') do (
    set "port_5000_found=1"
    set "pid_5000=%%b"
    echo [发现] 5000端口被PID !pid_5000! 占用
    echo [操作] 正在终止进程 !pid_5000!...
    taskkill /pid !pid_5000! /f >nul 2>&1
    if !errorlevel! equ 0 (
        echo [成功] 进程 !pid_5000! 已终止
    ) else (
        echo [警告] 无法终止进程 !pid_5000!，可能需要管理员权限
    )
)
...
for /f "tokens=2,5" %%a in ('netstat -ano ^| findstr ":3000.*LISTENING"') do (
    set "port_3000_found=1"
    set "pid_3000=%%b"
    echo [发现] 3000端口被PID !pid_3000! 占用
    echo [操作] 正在终止进程 !pid_3000!...
    taskkill /pid !pid_3000! /f >nul 2>&1
    if !errorlevel! equ 0 (
        echo [成功] 进程 !pid_3000! 已终止
    ) else (
        echo [警告] 无法终止进程 !pid_3000!，可能需要管理员权限
    )
)
...
if not exist "aispring" (
    echo [错误] 未找到后端目录 aispring
    pause
    exit /b 1
)

if not exist "vue-app" (
    echo [错误] 未找到前端目录 vue-app
    pause
    exit /b 1
)
