...
### 🛠️ Agent 终端助手下载中心上线 (2026-01-13)
- **全新下载大类组件**:
  - **AgentDownloadCenter**: 设计并上线了独立的 Agent 下载中心组件，提供统一的跨平台版本管理界面。
  - **Linux 版本发布 (v1.0.0)**: 正式发布首个 Linux 二进制版本，支持主流发行版，提供极简的命令行交互能力。
  - **版本路线图**: 规划了 Windows (v1.2.0) 和协作式 Agent 矩阵 (v2.0.0) 的演进路径，明确了产品长期发展目标。
- **AgentView 深度集成**:
  - **导航优化**: 在 Agent 专题页 Hero 区域新增了直接跳转至下载中心的快速通道。
  - **内容增强**: 新增了安装说明、安全保障及详细的版本演进计划，提升了用户获取工具的信任度与便利性。
- **静态资源部署**:
  - 在 `public/downloads` 目录下部署了 `xiaochen-agent-linux` 二进制文件，支持 Web 端直接下载。
