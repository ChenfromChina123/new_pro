...
### 🎨 界面图标、安全配置与公共资源组件 (2026-01-05)
- **公共文件下载组件**:
  - 新增 **公共资源** 模块，提供常用文件的下载服务。
  - 管理员可以通过网页端上传文件到指定目录（`public_files`），普通用户可浏览并下载。
  - 侧边栏新增 "公共资源" 入口，支持管理员权限动态显示。
  - **后端实现**: 增加了 `PublicFileController` 处理列表、下载和上传请求，并配置了 Spring Security 允许公开访问 GET 请求。
  - **前端实现**: 新增 `PublicFilesView.vue` 界面和 `publicFiles.js` Pinia 状态管理。
- **前端界面精简**:
  - 移除了 `ChatView.vue` 输入框工具栏中的无用组件，包括：截图（剪刀）、语音通话（电话）、语音输入（麦克风）以及上传附件（回形针）图标。
  - 优化了工具栏布局，使界面更加聚焦于核心聊天功能。
- **数据库兼容性修复**:
  - 修复了 `Admin` 实体类中 `is_active` 字段缺失导致的数据库插入失败问题。
  - 优化了 `User` 和 `Admin` 实体类的字段默认值配置，确保与 MySQL 数据库结构一致。
- **网站图标更新**: 更换了全新的 SVG 格式网站图标（Favicon），提升了视觉专业度。
- **敏感配置加密 (Jasypt)**: 
  - 集成了 **Jasypt** 加密框架，对 `application.yml` 中的数据库密码、邮件授权码、DeepSeek API Key 等敏感信息进行了加密处理（`ENC(...)` 格式）。
  - **安全性提升**: 配置文件中不再明文展示密钥，有效防止源码泄露导致的风险。
  - **灵活部署**: 支持通过环境变量 `JASYPT_PASSWORD` 动态传入解密盐值，兼顾开发便捷性与生产安全性。
  - **加密工具**: 提供 `JasyptUtils.java` 工具类，方便用户自行生成加密字符串。
- **头像上传功能修复**:
  - 修复了上传头像时因目录不存在导致的 500 错误，增加了自动创建目录的逻辑。
  - 修复了由于 Spring Security 配置不当导致的上传请求被错误拦截或 `CustomUserDetails` 为空的问题。
  - 明确了头像查看（GET）为公开权限，头像上传（POST）为需认证权限。
- **前端代码渲染优化**:
  - 修复了 AI 输出的代码块中包含特殊字符（如 `$`, `$$`）时被吞掉或渲染错误的问题。
  - 优化了 `ChatView.vue` 的 Markdown 渲染逻辑，集成了 `highlight.js` 语法高亮。
  - 增加了对未闭合代码块的自动修复逻辑，提升了流式输出过程中的展示稳定性。
...
- **Linux 一键部署增强**: 
  ...
  - 解决了 `git pull` 时的代码冲突问题，确保部署脚本与远程仓库保持一致。
- **前端界面精简**:
