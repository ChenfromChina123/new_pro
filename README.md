...
### 🛠️ SSE 异常处理优化 (2026-03-24)
- **问题描述**: 在 SSE (Server-Sent Events) 流式传输过程中，当客户端过早断开连接时，后端可能会抛出 `IOException: Connection reset by peer` 或 `AsyncRequestNotUsableException`，导致控制台出现大量错误日志。
- **优化方案**:
  - 在 `AiChatServiceImpl.java` 的 `sendChatResponse` 方法中，将 `IOException` 的处理从抛出 `RuntimeException` 修改为记录 `warn` 级别的日志，并优雅地终止当前发送操作，避免不必要的异常堆栈。
  - 在 `AiChatController.java` 的匿名用户处理逻辑中，当发送速率限制错误信息时，将 `IOException` 的处理从 `emitter.completeWithError(e)` 修改为记录 `warn` 级别的日志，并直接返回 `emitter`，以更静默的方式处理客户端断开连接。
- **效果**: 显著减少了因客户端断开连接导致的后端错误日志，提升了系统的健壮性和日志清晰度。

### 🖼️ 图片消息与联网搜索隐私展示重构 (2026-03-24)
- **图片输入区清理**: 点击“解释图片”后立即清空输入区图片预览，避免发送后残留在输入框区域。
- **消息显示策略**: 图片消息在实时与历史加载中统一只展示用户气泡内小图缩略图，不再展示 OCR 原文。
- **历史恢复能力**: 新增图片元数据编码/解码链路，保存聊天记录时持久化图片列表，重新加载会话时自动还原小图展示。
- **联网搜索脱敏**: 图片识别场景下联网搜索仅使用用户可见文本作为检索词，不再将 OCR 原文写入搜索面板与历史字段。
- **后端兼容扩展**: `/api/chat-records/save` 支持 `user_images` 字段，服务端自动转为图片元数据存储，前端无缝恢复。
...
### 💬 AI 对话建议按钮与首轮标题修复 (2026-03-14)
- **标题/建议模型对齐**:
  - 将会话标题与建议问题的异步生成模型改为与主对话链路一致的模型配置（`deepseek-v3.2`），修复因模型不一致导致的“建议按钮不出现、首轮标题不更新”问题。
- **会话列表标题优先级修复**:
  - 前端会话列表优先读取后端返回的 `session.title`，仅在缺失时回退 `last_message`，确保 AI 生成标题可见。
