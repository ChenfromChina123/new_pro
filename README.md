...
- **顶层组件集成 Linux 介绍**:
  - **Hero 区域重构**: 在 `AgentView.vue` 顶层 Hero 区域直接引入 Linux 支持介绍，提升了 Linux 版本的曝光度与获取效率。
  - **跨平台全量包 (v1.0.0)**: 将 Windows 发行版与 Linux 二进制文件集成到统一的 `xiaochen_agent_v1.0.0.zip` 压缩包中，实现一包支持多平台。
  - **下载体验优化**: 提供了“全平台完整包”与“Linux 独立版”两种下载选择，满足不同用户的部署需求。
- **全新下载大类组件**:
  - **AgentDownloadCenter**: 重新设计了下载中心布局，突出展示全平台集成能力与版本演进路线图。
- **静态资源更新**:
