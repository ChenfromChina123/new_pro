...
### 🛠️ Agent 终端助手详情页独立化与路由优化 (2026-01-13)
- **Agent 专题页独立化**:
  - **架构调整**: 将 Agent 详情页从 `AppLayout` 的子路由中移出，变为与首页平级的顶层路由。现在访问 `/agent` 将呈现完全独立的 Landing Page 风格，不再带有侧边栏，提供沉浸式阅读体验。
  - **视觉一致性**: 为 Agent 专题页添加了与首页一致的顶部透明导航栏和页脚，确保用户在不同页面间切换时的视觉连贯性。
  - **交互增强**: 专题页现已具备完整的移动端适配和主题切换功能。
