...
### 🧩 系统架构与性能优化
- **Keep-Alive 生命周期优化**: 为所有主要模块（语言学习、云盘、AI问答）引入了 `onActivated` 和 `onDeactivated` 生命周期钩子。解决了在 `keep-alive` 模式下，组件切换后全局事件监听器（如窗口缩放、鼠标移动、点击外部关闭菜单等）和定时器未能正确释放的问题，显著降低了内存占用并提升了系统稳定性。
- **模块切换显示修复**: 修复了点击“语言学习”模块后切换到其他模块时可能出现的显示异常。通过优化 `AppLayout.vue` 的路由视图 `key` 策略（由 `fullPath` 改为 `name`）和完善生命周期管理，确保了组件状态的正确恢复与清理。
