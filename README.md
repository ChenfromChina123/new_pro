...
- **专用文件夹系统**:
  ...
  - **多功能终端面板**:
    - **文件管理**: 集成可视化文件浏览器，支持查看、导航及文件操作，双击即可快速切换目录。
    - **需求文档系统**: 内置需求文档生成与管理功能，支持版本控制（历史记录查看与回滚）及 AI 辅助生成。
    - **任务清单**: 支持 AI 生成并实时更新的任务清单展示，清晰呈现复杂任务的执行进度。
  - **路径简化显示**: 终端命令行自动简化路径显示（仅显示相对路径），优化视觉体验。
...
- **流式输出解析优化**: 
  - **SSE 协议增强**: 修复了前端对 SSE (Server-Sent Events) 协议解析不完整的问题，现在能够正确识别和处理 `event: session_update` 等命名事件，实现会话标题的实时更新。
  - **数据规范化**: 引入了数据规范化层，自动处理后端 `snake_case` 与前端 `camelCase` 字段的不一致（如 `session_id` 映射为 `sessionId`），彻底解决了点击会话或新建会话“无反应”的问题。
  - **Reasoning 内容支持**: 适配了 DeepSeek Reasoner 等模型的推理内容展示，支持流式显示 `reasoning_content`。
  - **JSON 鲁棒性**: 增强了对 AI 输出 JSON 格式的解析鲁棒性，支持从混合文本中提取指令块，并修复了流式输出过程中 JSON 未闭合导致的显示空白问题。
  - **后端日志与追踪**: 在终端聊天接口增加了详细的调试日志，便于追踪 AI 请求与响应流。
- **UI 交互与反馈**:
  - **全方位可折叠布局**: 实现了“每一个栏位皆可折叠”的设计理念。左侧会话列表、右侧工具面板（终端、文件、需求）均支持一键折叠，最大化中间核心对话区域的视野。
  ...
  - **实时流式解析**: 针对终端 Agent 输出的 JSON 结构进行了流式解析优化。现在即使在生成复杂的指令 JSON 时，其中的 `message` 文字内容和 `thought` 思考过程也能实时、同步地展现在界面上，彻底告别“生成完才显示”的等待感。
