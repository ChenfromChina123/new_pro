...
- **模型选择器与深度思考逻辑优化**:
  - 重构了模型选择器为品牌分类下拉框（支持豆包、DeepSeek），移除了图标并调大字体，交互更加丝滑。
  - 实现了深度思考自动切换模型逻辑：开启“深度思考”时自动切换至 DeepSeek Reasoner 模型，关闭时切回标准模型。
- **云盘组件深度集成**:
  - 实现了云盘与聊天界面的无缝切换：点击侧边栏“云盘”导航时，主内容区切换为云盘视图，同时保持左侧导航栏状态不变。
  - 适配了云盘组件的 UI 风格，使其与聊天界面保持一致（侧边栏宽度 300px，统一的圆角和间距）。
  - 在聊天侧边栏集成了文件夹管理功能，支持直接在侧边栏创建、删除和重命名文件夹，操作更加直观。
- **全局导航架构重构**:
...
- **云盘组件深度集成**:
  - 将云盘功能深度集成进 AI 问答界面（ChatView.vue），支持在侧边栏通过导航无缝切换“AI 问答”与“云盘”模式。
  - 实现了侧边栏状态共享：切换到云盘模式时，侧边栏保持不变，仅将内容区域切换为文件夹树，实现了图 3 风格的统一交互体验。
  - 在侧边栏集成了文件夹管理功能：支持直接在侧边栏创建、重命名和删除文件夹。
  - 优化了云盘主视图布局：在集成模式下自动隐藏冗余的内部侧边栏，使界面更加整洁。
  - 引入了弹窗模糊背景（backdrop-filter: blur(4px)）和现代化动画效果，提升了操作反馈感。
