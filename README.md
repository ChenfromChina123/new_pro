...
### 🛠️ Agent 终端助手发布 (2026-01-12)
- **新增 Agent 终端助手下载 (Windows 版)**:
  - **首页快捷下载**: 在欢迎页 (Landing Page) 的 Hero 区域新增了“下载 Agent 终端助手 (Windows)”按钮。
  - **核心特性展示**: 在功能列表中新增了“Agent 终端助手 (Windows)”卡片，明确其专为 Windows 环境优化。
  - **全局入口**: 在页脚 (Footer) 的支持栏目中同步增加了 Windows 版本的下载链接。
  - **资源部署**: 将终端助手压缩包 (`xiaochen_terminal.zip`) 部署至前端公共目录，支持高速直链下载。
