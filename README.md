...
- **UI 深度优化**:
  ...
  - **会话管理**: 在侧边栏增加了直观的删除按钮，并配有二次确认逻辑，防止误删。

### 🧩 交互稳定性**滚动逻辑优化**: 修复了聊天内容滚动不准确的问题。现在通过 `setTimeout` 和 `triggerScrollToBottom` 助手函数，确保在会话切换、页面刷新以及 DOM 完全渲染（包含数学公式）后，能够准确、可靠地滚动到消息列表最底部。
