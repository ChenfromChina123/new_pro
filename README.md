...
- **流式渲染与性能优化**: 引入前端流式分片刷新机制，减少高频 Markdown/公式渲染开销，确保生成内容在极低延迟下实时展示，并在长文本输出场景保持界面流畅与可交互。
- **滚动锚定体验**: 当用户停留在底部时自动跟随新内容；当用户上滑查看历史消息时，新内容更新不会触发滚动跳动或位置异常。
...
- **引导问题视角优化**: 调整了建议问题的生成逻辑，确保问题完全站在**用户视角**（如“你可以帮我做什么？”），而非 AI 视角（如“我能帮你什么？”），提升交互代入感。
