...
- **云盘路径导航优化**: 
  - 修复了面包屑路径显示不全的问题，现在可以正确显示完整的文件夹层级（如 `123 > 写作`）。
  - 根据用户需求，在进入子目录后自动隐藏“根目录/全部文件”节点，使路径显示更加聚焦。
- **文件夹树 UI 优化**: 
  - 修复了点击父目录时子目录也会显示蓝色边框的问题。通过使用 CSS 直接子代选择器 (`>`)，确保只有当前选中的文件夹项会被高亮。
  - 将文件夹的操作按钮（重命名、删除）合并为一个悬浮显示的选项卡，仅在鼠标悬停在文件夹上时平滑显示，使界面更加整洁并减少误触。
- **代码质量与安全性优化**:
