...
- **新增 Agent 终端助手下载**:
  - **首页快捷下载**: 在欢迎页 (Landing Page) 的 Hero 区域新增了“下载 Agent 终端助手”按钮，方便用户快速获取本地工具。
  - **核心特性展示**: 在功能列表中新增了“Agent 终端助手”卡片，详细介绍了其本地文件处理、自动化脚本运行和高效命令行交互的能力。
  - **全局入口**: 在页脚 (Footer) 的支持栏目中同步增加了下载链接，确保用户在任何位置都能轻松找到。
