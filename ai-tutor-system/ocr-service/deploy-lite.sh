...
install_python_deps() {
    print_info "安装Python依赖（ddddocr超轻量版）..."
...
deploy_service() {
    ...
    cp -r $SCRIPT_DIR/app_lite.py $INSTALL_DIR/app.py
    cp -r $SCRIPT_DIR/requirements-lite.txt $INSTALL_DIR/requirements.txt
...
[Unit]
Description=OCR Service Lite - ddddocr (80MB)
...
print_deploy_info() {
    ...
    echo ""
    echo "返回格式: {\"success\": true, \"text\": \"识别的文字\"}"
    echo ""
    echo "管理命令:"
