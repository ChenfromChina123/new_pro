...
#### 命令执行结果持久化与刷新消失修复
- **问题**: 终端命令执行结果（stdout, stderr, exit_code）仅保存在内存中，刷新页面后会消失。
- **修复**: 
  - **数据库扩展**: 在 `ChatRecord` 实体中增加了 `exit_code`, `stdout`, `stderr` 字段，支持统一持久化。
  - **后端增强**: 更新了 `ChatRecordService` 和 `TerminalController`，在保存终端记录时同步保存工具执行的具体结果。
  - **前端同步**: `TerminalView.vue` 在工具执行完成后，通过 `saveMessage` 立即将结果持久化到数据库。
  - **历史回溯**: `terminal.js` store 在加载历史记录时，会自动解析持久化的工具结果并还原到终端面板，确保刷新后执行记录依然可见。
  - **逻辑优化**: 统一了前端对 `exit_code` 的处理逻辑，确保成功/失败状态判断准确。
