...
export default function NewsletterForm({ title, apiUrl = '/api/newsletter' }: NewsletterFormProps) {
  ...
  const subscribe = async (e: ChangeEvent<HTMLFormElement>) => {
    e.preventDefault()
    ...
    if (error) {
      setError(true)
      setMessage(translations.newsletter.invalidEmail)
      return
    }
  ...
  return (
    <div>
      <div className="pb-1 text-lg font-semibold text-gray-800 dark:text-gray-100">
        {title || translations.newsletter.title}
      </div>
      <form className="flex flex-col sm:flex-row" onSubmit={subscribe}>
        <div>
          <label htmlFor="email-input">
            ...
            <input
              ...
              placeholder={
                subscribed
                  ? translations.newsletter.subscribed
                  : translations.newsletter.placeholder
              }
              ref={inputEl}
              required
              type="email"
        ...
        <div className="mt-2 flex w-full rounded-md shadow-sm sm:mt-0 sm:ml-3">
          <button
            ...
            disabled={subscribed}
          >
            {subscribed ? translations.newsletter.thanks : translations.newsletter.button}
          </button>
        </div>
      </form>
      {error && (
